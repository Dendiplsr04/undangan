...
export const comment = (() => {
    ...
    const onNullComment = () => {
        const desc = lang
            .on('id', '✨ Yuk, tulis ucapan & doa untuk kami! Ditunggu ya~ 💕')
            .on('en', '✨ Yuk, tulis ucapan & doa untuk kami! Ditunggu ya~ 💕')
            .get();
    ...
    const send = async (button) => {
        ...
        if (nameValue.length === 0) {
            util.notify('Nama wajib diisi ya!').warning();
        ...
        if (!id && presence && presence.value === '0') {
            util.notify('Pilih dulu dong, datang atau nggak?').warning();
            return;
        }
        ...
        if (gifIsOpen && !gifId) {
            util.notify('Gif-nya belum dipilih nih!').warning();
            return;
        }
    ...
    const cancel = async (button, id) => {
        ...
        if (gif.isOpen(id) && ((!gif.getResultId(id) && isChecklist === isPresent) || util.ask('Yakin mau batal?'))) {
            await gif.remove(id);
            removeInnerForm(id);
            return;
        }
        ...
        const form = document.getElementById(`form-inner-${id}`);
        if (form.value.length === 0 || (util.base64Encode(form.value) === form.getAttribute('data-original') && isChecklist === isPresent) || util.ask('Yakin mau batal?')) {
            removeInnerForm(id);
            return;
        }
